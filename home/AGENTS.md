...
- `~/.claude/settings.json` is regenerated on every `darwin-rebuild switch`
  by a `home.activation` script (`dotfiles-amway/home/claude/merge-settings.sh`)
  that `jq`-merges `~/dotfiles/home/claude/settings.json` (personal base) with
  `dotfiles-amway/home/claude/settings.work.json` (work overlay). Edit one of
  those two sources, never the merged output - it gets overwritten on rebuild.
- `~/.agents/instructions/INSTALLATIONS.md` is likewise regenerated by
  `dotfiles-amway/home/instructions/merge-installations.sh`, merging
  `~/dotfiles/home/instructions/INSTALLATIONS.md` (personal) with
  `dotfiles-amway/home/instructions/INSTALLATIONS-WORK.md` (work addendum).
- If a symptom looks like "I edited a dotfile in `~/.config` or `~/.claude`
  etc. but it keeps reverting" or "the change isn't taking effect", check
  `dotfiles-amway/home.nix` for a `home.activation` script or `home.file`
  entry targeting that path before assuming the edit failed.
