...
tasks-axi is installed globally and pinned to an exact version by dotfiles (see ~/.claude/plans/memoized-mixing-quilt.md) - invoke it directly with `tasks-axi <command>`.
If tasks-axi output shows a follow-up command starting with `tasks-axi`, run it as `tasks-axi ...` instead.
...
1. Run `tasks-axi` with no arguments for a dashboard of the current backlog - in flight work, queued work with blockers, and suggested next commands.
...
Run `tasks-axi --help` for global flags, or `tasks-axi <command> --help` for per-command usage.
...
Do not run `tasks-axi update` - it self-updates via npm and would drift from
...
`tasks-axi update --check` (read-only, does not install) is still fine to run.
